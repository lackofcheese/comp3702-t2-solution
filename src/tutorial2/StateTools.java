...
public class StateTools {
	...
	/**
	 * Returns a state interpolated linearly between s0 and s1, where s0 corresponds to t=0
	 * and s1 corresponds to t=1. 
	 * @param s0 the first state.
	 * @param s1 the second state.
	 * @param t  the interpolation parameter. 
	 * @return a state at value t between s0 (t=0) and s1 (t=1).
	 */
	public static RobotArmState interpolate(RobotArmState s0, RobotArmState s1, double t) {
		double angle1_0 = s0.getAngle1();
		double angle1_1 = s1.getAngle1();
		double angle2_0 = s0.getAngle2();
		double angle2_1 = s1.getAngle2();
		...
		return new RobotArmState(s0.getLength1(), s0.getLength2(), 
				angle1_0 * (1-t) + angle1_1 * t,
				angle2_0 * (1-t) + angle2_1 * t);
	}
	 ...
	 */
	public static boolean hasDirectPath(RobotArmState s0, RobotArmState s1, List<Obstacle> obstacles) {
